...
from app.models.policy import Policy, PolicyCreate, PolicyResponse, PolicyUpdate
...
@router.get(
    ...
    response_model=List[Policy],
    status_code=status.HTTP_200_OK,
    summary="Get Wallet Policies (short path alias)",
)
async def get_wallet_policies_short(
    wallet_id: UUID = Path(..., description="Wallet identifier")
) -> List[Policy]:
    """Alias for /wallet/{wallet_id}/policies — returns all policies for a wallet."""
    return await get_wallet_policies(wallet_id)
...
async def update_policy(
    ...
    try:
        return await policy_service.update_policy(policy_id, updates.model_dump(exclude_none=True))
    except Exception as e:
        if "PolicyNotFoundException" in str(type(e).__name__):
