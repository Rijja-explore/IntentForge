...
async def get_blockchain_audit_log(
    ...
    """
    Get blockchain audit log entries formatted for the frontend explorer.
    """
    try:
        logs = blockchain_audit_service.get_audit_log(limit=limit, event_type=event_type)

        # ── Map internal event types → frontend type/status ──────────────
        EVENT_TYPE_MAP = {
            "TRANSACTION_APPROVED":  ("transaction", "APPROVED"),
            "TRANSACTION_VIOLATED":  ("violation",   "VIOLATION"),
            "TRANSACTION_BLOCKED":   ("transaction", "BLOCKED"),
            "POLICY_CREATED":        ("policy",      "REGISTERED"),
            "POLICY_UPDATED":        ("policy",      "REGISTERED"),
            "CLAWBACK_EXECUTED":     ("clawback",    "CLAWBACK"),
        }

        recent_events = []
        for entry in reversed(logs):  # newest first
            ev_type, ev_status = EVENT_TYPE_MAP.get(
                entry.get("event_type", ""), ("transaction", "UNKNOWN")
            )

            # Extract a short truncated txId from blockchain_tx_id or metadata
            raw_tx = (
                entry.get("blockchain_tx_id")
                or entry.get("metadata", {}).get("transaction_id", "")
                or entry.get("metadata", {}).get("policy_id", "")
                or "unknown"
            )
            tx_id = f"{str(raw_tx)[:6]}…{str(raw_tx)[-4:]}" if len(str(raw_tx)) > 12 else str(raw_tx)

            # Extract wallet
            wallet_raw = entry.get("metadata", {}).get("wallet_id", "unknown")
            wallet_short = f"{str(wallet_raw)[:8]}…{str(wallet_raw)[-3:]}" if len(str(wallet_raw)) > 12 else str(wallet_raw)
            ...
            recent_events.append({
                ...
                "wallet":    wallet_short,
                "timestamp": ts_str,
                "hash":      entry.get("blockchain_hash") or entry.get("data_hash", ""),
            })

        # ── Build statistics ──────────────────────────────────────────────
        all_logs = blockchain_audit_service.get_audit_log(limit=10000)
        statistics = {
            "total_policies":            sum(1 for e in all_logs if e.get("event_type") in ("POLICY_CREATED", "POLICY_UPDATED")),
            "total_transactions_logged": sum(1 for e in all_logs if "TRANSACTION" in e.get("event_type", "")),
            "total_approved":            sum(1 for e in all_logs if e.get("event_type") == "TRANSACTION_APPROVED"),
            "total_blocked":             sum(1 for e in all_logs if e.get("event_type") in ("TRANSACTION_VIOLATED", "TRANSACTION_BLOCKED")),
            "total_violations":          sum(1 for e in all_logs if e.get("event_type") == "TRANSACTION_VIOLATED"),
            "total_clawbacks":           sum(1 for e in all_logs if e.get("event_type") == "CLAWBACK_EXECUTED"),
        }

        return APIResponse(
            success=True,
            message=f"Retrieved {len(recent_events)} audit log entries",
            data={
